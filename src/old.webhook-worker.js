// src/webhook-worker.js — loop-safe, top-level attributes, robust retries
...
function normalizeEndpoint(input){
  ...
  // remove ".apps." (case-insensitive)
  u.hostname = u.hostname.replace(/\.apps\./i, ".");
  // force Logs v2 ingest path
...
self.onmessage = async (event) => {
  ...
  const sendChunkRecord = async (rec) => {
  ...
  const runSequentialOnce = async () => {
    for (let i=0; i<total; i += batchSize){
      ...
      for (const item of chunk){
        ...
        const rs = await sendChunkRecord(rec);
        if (!rs.ok){
          if (rs.cancelled) return false;
          self.postMessage({ type:"ERROR", error:`HTTP ${rs.status}: ${rs.text}` });
          return false;
        }
      }
      if (delayMs > 0){
  ...
  try{
    if (mode === "sequential"){
      ...
      return loop ? undefined : finish('done');
    } else {
      do {
        self.postMessage({ type:"PROGRESS", progress: 0 });
